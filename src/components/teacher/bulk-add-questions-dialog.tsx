...
import { generateQuizQuestions } from '@/ai/flows/generate-quiz-questions-flow'; // Import the new AI flow
import { Button } from '@/components/ui/button';
import { Textarea } from '@/components/ui/textarea';
import { Input } from '@/components/ui/input'; // For AI topic and num questions
...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'; // Added import
...
export default function BulkAddQuestionsDialog({
  ...
  const { subject } = useSettings(); // Get current subject
  const [isProcessing, setIsProcessing] = useState(false);

  // State for AI Generation
  const [aiTopic, setAiTopic] = useState('');
  const [aiNumQuestions, setAiNumQuestions] = useState('3'); // Default to 3 questions
  ...
  const parseBulkText = (text: string): { questions: AddMCQQuestionFormValues[], errors: string[] } => {
    ...
    questionBlocks.forEach((block, index) => {
      const lines = block.trim().split('\n').map(line => line.trim()).filter(line => line.length > 0); // Filter out empty lines that might result from multiple blank lines
      if (lines.length !== 7) {
  ...
  const handleManualSubmit = async (values: BulkAddFormValues) => {
    ...
    try {
      ...
      if (result.errorCount > 0) {
         toast({
            variant: "destructive",
            title: `Partial Success: ${result.successCount} questions added.`,
            description: (
                <ScrollArea className="h-20">
                    <p>{result.errorCount} questions failed:</p>
                    <ul className="list-disc list-inside">
      ...
      onQuestionsAdded();
      form.reset();
      setShowAiGenerationFields(false);
      setAiTopic('');
      setAiNumQuestions('3');
      onOpenChange(false);
    } catch (error: any) {
  ...
  const handleAiGenerate = async () => {
    if (!quizDetails) {
        toast({ variant: "destructive", title: "Error", description: "Quiz details missing for AI generation." });
        return;
    }
    if (!aiTopic.trim()) {
        toast({ variant: "destructive", title: "Input Required", description: "Please enter a topic for AI question generation." });
        return;
    }
    const numQs = parseInt(aiNumQuestions, 10);
    if (isNaN(numQs) || numQs < 1 || numQs > 10) {
        toast({ variant: "destructive", title: "Invalid Number", description: "Number of questions must be between 1 and 10." });
        return;
    }
    ...
    try {
        const aiInput = {
            subject: subject, // Get current subject from settings context
            quizTitle: quizDetails.title,
            classContext: "Advanced Level students in Sri Lanka", // Example context, can be made dynamic
            topic: aiTopic,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        ...
        <Button variant="outline" onClick={() => setShowAiGenerationFields(!showAiGenerationFields)} className="my-3 w-full sm:w-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleManualSubmit)} className="space-y-4 py-2">
