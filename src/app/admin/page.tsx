...
export default function AdminDashboardPage() {
  ...
  const handleDownloadTeacherReport = (teacherInfo: TeacherVoucherStat) => {
    ...
    if (teacherVouchersLast30Days.length > 0) {
      teacherVouchersLast30Days.forEach(voucher => {
        ...
        let voucherLine = `Code: ${voucher.id} | Batch: ${voucher.batchId} | Credits: ${voucher.credits} | Status: ${voucher.status}`;
  ...
  const handleDownloadAllVouchersReport = () => {
    ...
    creditVouchers.forEach((voucher, index) => {
      if (yPos > pageHeight - margin - (lineSpacing * 8)) { // Estimate space needed for one entry
        doc.addPage();
        yPos = margin;
        doc.setFontSize(12);
      ...
      doc.text(`  Batch ID: ${voucher.batchId}`, margin, yPos); yPos += lineSpacing;
